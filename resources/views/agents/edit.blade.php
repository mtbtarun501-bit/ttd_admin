...
<div class="container-fluid" style="padding: 40px;">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 style="color:var(--temple-dark-brown); font-weight:700;"><i class="fas fa-user-edit me-2"></i> Edit Agent: {{ $agent->name }}</h4>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="agent_type" class="form-select @error('agent_type') is-invalid @enderror" required>
                            ...
                            <option value="{{ \App\Models\Agent::TYPE_IN_PARTNER }}" {{ old('agent_type', $agent->agent_type) == \App\Models\Agent::TYPE_IN_PARTNER ? 'selected' : '' }}>In Partner</option>
                            <option value="{{ \App\Models\Agent::TYPE_OUT_PARTNER }}" {{ old('agent_type', $agent->agent_type) == \App\Models\Agent::TYPE_OUT_PARTNER ? 'selected' : '' }}>Out Partner</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="0" {{ old('status', $agent->status) === false || old('status', $agent->status) === 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>

                    <div class="col-12">
                        <label class="form-label fw-bold">Remarks</label>
                        <textarea name="remarks" class="form-control" rows="3">{{ old('remarks', $agent->remarks) }}</textarea>
                    </div>
                </div>
